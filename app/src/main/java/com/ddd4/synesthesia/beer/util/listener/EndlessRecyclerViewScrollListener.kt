package com.ddd4.synesthesia.beer.util.listener
...
abstract class EndlessRecyclerViewScrollListener(private val layoutManager: RecyclerView.LayoutManager?) :
    ...
    override fun onScrolled(view: RecyclerView, dx: Int, dy: Int) {
        ...
        // If the total item count is zero and the previous isn't, assume the
        // list is invalidated and should be reset back to initial state
        scrollLog("${totalItemCount < previousTotalItemCount} : $totalItemCount < $previousTotalItemCount")
        ...
        // changed, if so we conclude it has finished loading and update the current page
        // number and total item count.
        scrollLog("${loading && totalItemCount > previousTotalItemCount} : $totalItemCount > $previousTotalItemCount")
        ...
        // If we do need to reload some more data, we execute onLoadMore to fetch the data.
        // threshold should reflect how many total columns there are too
        scrollLog("${!loading && lastVisibleItemPosition + visibleThreshold > totalItemCount} : $lastVisibleItemPosition + $visibleThreshold > $totalItemCount")
